...
- chunking 으로 분리된 메시지 처리 방식을 `config.json` 의 `contextTruncate` 로 제어하며, 기본값은 `false` 로 모든 chunk 를 context 에 포함한다. `true` 로 설정하면 각 메시지에서 첫 번째 chunk 만 context 에 남기고 나머지는 버린다.
